...
// Repositori fix on es pengen totes les captures de tiquets, independentment
// del projecte a qui pertany el tiquet. Evita que cada captura sigui un
// commit directe al repo de codi del projecte (i que calgui fer `git pull`
// cada cop que algú n'adjunta una).
const SCREENSHOTS_REPO_OWNER = process.env.SCREENSHOTS_REPO_OWNER;
const SCREENSHOTS_REPO_NAME = process.env.SCREENSHOTS_REPO_NAME;

function sanitizeFilename(name) {
  return name.replace(/[^a-zA-Z0-9._-]/g, '-').slice(-80);
}

// Puja una captura de pantalla al repositori dedicat a adjunts (SCREENSHOTS_REPO_*)
// via la Contents API de GitHub (a la carpeta .ticket-uploads/) i retorna la
// URL pública de descàrrega de la imatge.
async function uploadScreenshotToGithub(file) {
  const path = `.ticket-uploads/${Date.now()}-${crypto.randomUUID().slice(0, 8)}-${sanitizeFilename(file.originalname)}`;
  const ghResponse = await fetch(
    `https://api.github.com/repos/${SCREENSHOTS_REPO_OWNER}/${SCREENSHOTS_REPO_NAME}/contents/${path}`,
    {
      method: 'PUT',
      headers: {
...
// Extreu owner/repo/branch/path d'una download_url de la Contents API
// (https://raw.githubusercontent.com/{owner}/{repo}/{branch}/{path}).
function parseScreenshotUrl(url) {
  const match = (url || '').match(/^https:\/\/raw\.githubusercontent\.com\/([^/]+)\/([^/]+)\/[^/]+\/(.+)$/);
  if (!match) return null;
  return { owner: match[1], repo: match[2], path: match[3] };
}

// Elimina una captura del repositori d'adjunts en eliminar el tiquet que la
// tenia. Si ja no existeix o la URL no es reconeix, no fa res (no bloqueja
// l'eliminació del tiquet).
async function deleteScreenshotFromGithub(url) {
  const parsed = parseScreenshotUrl(url);
  if (!parsed) return;
  ...
  try {
    ...
    const delRes = await fetch(
      `https://api.github.com/repos/${owner}/${repo}/contents/${path}`,
      {
        ...
        body: JSON.stringify({ message: 'Elimina captura de tiquet', sha: data.sha })
      }
    );
    if (!delRes.ok) {
      const errText = await delRes.text();
      console.error('Error eliminant captura de GitHub:', delRes.status, errText);
    }
  } catch (err) {
    console.error('Error eliminant captura de GitHub:', err.message);
  }
}
