...
async function sendSetPasswordEmail({ to, nom, actionLink }) {
  await send({
    to,
    subject: 'Activa el teu accés — Portal de tiquets UAUU',
    html: `
      <p>Hola ${nom},</p>
      <p>Un administrador ha aprovat el teu accés al portal de tiquets. Crea la teva contrasenya per començar:</p>
      <p><a href="${actionLink}">Crear la meva contrasenya</a></p>
      <p>Si no has sol·licitat aquest accés, ignora aquest correu.</p>
    `
  });
}

async function sendPasswordRecoveryEmail({ to, actionLink }) {
  await send({
    to,
    subject: 'Recupera la teva contrasenya — Portal de tiquets UAUU',
    html: `
      <p>Hem rebut una petició per restablir la teva contrasenya al portal de tiquets.</p>
      <p><a href="${actionLink}">Crear una nova contrasenya</a></p>
      <p>Si no has estat tu, ignora aquest correu; la teva contrasenya actual seguirà funcionant.</p>
    `
  });
}
